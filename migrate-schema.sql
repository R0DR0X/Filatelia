...
-- Note: Extensions already enabled via setup-db.mjs

-- ============================================
-- 1. UPDATE COUNTRY TABLE
-- ============================================

-- Add new columns to Country table
...
-- Catalog numbers (already exist as StampCatalogNumber model)
-- Adding direct columns for quick access
...
ALTER TABLE "Stamp" ADD COLUMN IF NOT EXISTS "issueDate" TIMESTAMP WITH TIME ZONE;
...
ALTER TABLE "Stamp" ADD COLUMN IF NOT EXISTS "marketPriceUpdatedAt" TIMESTAMP WITH TIME ZONE;
...
-- Semantic search (already added by setup-db.mjs)
-- embedding vector(1536) - already added
-- search_vector tsvector - already added
...
CREATE TABLE IF NOT EXISTS "PriceHistory" (
  ...
  "stampId" UUID REFERENCES "Stamp"(id) ON DELETE CASCADE,
...
-- ============================================
-- 5. CREATE COLLECTION TABLE (if not exists from Prisma)
-- ============================================
...
-- GIN index for full-text search (if search_vector column exists)
DO $$
BEGIN
  IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'Stamp' AND column_name = 'search_vector') THEN
...
-- Vector index for embedding similarity search (if embedding column exists)
DO $$
BEGIN
  IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'Stamp' AND column_name = 'embedding') THEN

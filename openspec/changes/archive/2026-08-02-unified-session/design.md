...
| Admin transport | Next proxy `/api/admin/[...path]`: verify `fp_session`, MUST re-validate current admin role from D1 (not trust the baked session claim), then forward to Worker with `X-Admin-Token` (secret, constant-time compare in Worker). Re-validation is critical: it makes revocation of admin status immediate rather than caching permission for up to 30 days. | Rewrite admin CRUD in Next (Worker endpoints touch R2/import — too large); new Bearer token in localStorage (keeps XSS-readable credential); trust the session role claim (caches permission, violates revocation requirement) | Minimal diff with re-validation; httpOnly cookie never leaves origin; Worker endpoints unchanged internally; revocation is effective |
...
Admin access (MUST re-validate role from D1):

    Browser      Next proxy         D1              Worker
      │ GET /admin/X │                │                │
      │──────────────►│ verify fp_session              │
      │               │ check `role` claim             │
      │               │ SELECT UserRole/Role by id     │
      │               │────────────────────────────────►│
      │               │◄────── current role ───────────│
      │               │ if role ≠ admin: 403           │
      │               │ if role = admin:               │
      │               │ GET /admin/X + X-Admin-Token   │
      │               │────────────────────────────────►│
      │               │◄────── response ───────────────│
      │◄──────────────│ (if revoked mid-session:       │
      │ 403           │  next request re-queries D1    │
      │               │  and denies immediately)       │
...
| `filatelia-web/src/app/api/admin/[...path]/route.ts` | Create | Session-gated proxy → Worker with `X-Admin-Token`, MUST re-validate admin role from D1 |
...
- **CRITICAL FOR REVOCATION**: if the admin proxy does NOT re-validate the role claim against D1, a revoked admin keeps access for up to 30 more days (the sliding renewal window). Re-validation is load-bearing.
...
- [x] Admin role revocation mechanism (E3 extension): DECIDED — proxy MUST re-query D1 before forwarding, not trust the session role claim.

...
join at session-issuance time. **E3 EXTENSION: The proxy MUST re-validate the
role claim against D1 on every request, not trust the baked token value,
to make revocation of admin status immediate.**
...
## Archive note (E3 extension)

Migration 0008 (`create_site_visit.sql`) was added to the runbook during E2
(closure of `/analytics/visit`). The 90-day absolute session cap was added
during E3 as a product decision to make revocation effective: unbounded
sliding renewal meant a session, and the role claim baked into it, could
live indefinitely, so a revoked admin would retain access for up to 30 more
days after removal. The cap makes revocation eventually total.

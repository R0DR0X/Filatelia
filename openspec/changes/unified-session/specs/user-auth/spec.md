# User Auth Specification (Delta — new capability)
...
never read from a `role` column on `User` (no such column exists).
...
- GIVEN an authenticated session with `role: admin`
...
to `fp_session` (per rollback ordering in the proposal).
